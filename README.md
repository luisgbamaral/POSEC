...
(STGCN/SAEA, Graph-WaveNet, STHSL) for crime forecasting. POSEC fits a per-cell
Poisson-GLM that treats the backbone as a covariate, then selects how much
spatial-lag correction each cell receives via a validation-tuned **dose**
(global / per-node / per-node Pareto knee on loss × |LISA|) and a per-cell
**gate**. Evaluated on three crime datasets (São Paulo, Porto Alegre, Bahía)
with a unified discrete log score (ALS), PAI / hit-rate, Moran's I and
...
critical analysis, see [docs/RESULTS.md](docs/RESULTS.md).

> The ST-AR residual hybrids live in a separate repo (**sthyb**). This repo
> covers only the POSEC / GUARD IA calibrator (plus the base distributions used
...
```bash
conda create -n posec python=3.9 -y && conda activate posec
...
|---|---|---|---|
| `python scripts/run_probabilistic.py` | `results/probabilistic/als_master.csv` | main results table (ALS/MAE/RMSE/MI/PAI per method) | ~10 min |
...
| | `per_node.csv` | per-cell errors, POSEC doses & gates (maps) | (same run) |
| `python scripts/run_spatial_diag.py` | `spatial_diag.csv` + `figs/*.png` | Pesaran CD, hop correlograms, eigenvalue/ECM figures (base vs POSEC) | ~5 min |

A `README.md` describing every column is written next to the CSVs.
Methods scored (9): `base+{Poisson,NB,Gauss}`, `guardia{,-nodec,-lisac}{,+NB}`.

## 6. Determinism

- All stochastic steps are seeded (`np.random.seed(0)` for the randomized PIT;
  permutation nulls in `spatial_diag` likewise).
- Estimators (GLM-IRLS, grid MLE) are deterministic given the data.
- Caveat: multi-threaded BLAS / parallel GLM fitting can flip floating-point
  ...
  OPENBLAS_NUM_THREADS=1` and `GUARDIA_NJOBS=1` in `posec/config.py` (slower).

## 7. Configuration

Everything lives in **`posec/config.py`**: cities, backbones, train/val/test
split sizes (110/110 days), NB dispersion grid, GUARD IA gate loss (`GUARDIA_GATE`),
EB pool min nodes / parallel jobs, and all paths. Changing an experiment =
