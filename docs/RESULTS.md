...
> **Scope note.** This file records the original *joint* study, where POSEC
> (GUARD IA) and the ST-AR residual hybrids were evaluated together. The code
> has since been split: the hybrids now live in the **sthyb** repo. The hybrid
> rows and Part A below are kept here for reference (real numbers from the joint
> run) but belong to sthyb; the POSEC-only tables must be regenerated from this
> repo's `run_probabilistic.py` (9 methods: base distributions + `guardia*`) for
> a standalone paper. In this repo POSEC is the headline method, not a contender
> against the hybrids.
