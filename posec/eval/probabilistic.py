...
For every city x backbone in config, scores three methods on the held-out test:
  ...
  posec              POSEC: per-cell Poisson calibration with a per-node
...
import pandas as pd
import sys
from posec.config import ALPHA_GRID, BACKBONES, BATCH, CITIES, GATE_FRAC, GATE_LOSS, CKPT_DIR, DATA_DIR, N_TEST, N_VAL, OUT_DIR
...
PROPOSED = 'posec'   # POSEC (per-node Pareto-optimal dose, NB-wrapped)
...
def main():
    ...
    for ds, N in CITIES:
        ...
        for bk in BACKBONES:
            ...
            gres = calibrate(y_tr, p_tr, y_va, p_va, y_te, p_te, Wr, N, gate_frac=GATE_FRAC)
            ...
            cs = gres['cstar_lisa']
            print(f"  posec gate={GATE_LOSS} | best_c={gres['best_c']:.2f}  "
            ...
            pn['posec_s_i'] = gres['lisapareto'][2]
...
- **als_master.csv** — city x backbone x method: ALS_discrete (unified discrete
  ...
  baselines) and posec (POSEC — the proposed per-cell Poisson
...
- **per_node.csv** — per node: MAE/RMSE for base and every method, plus the POSEC
  per-node dose (cstar_lisa) and gate (posec_s_i) for dose maps.

## Notes
- POSEC gate/Pareto loss is set by GATE_LOSS in config ('mse' here).
